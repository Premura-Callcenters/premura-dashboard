...
export default function TicketDashboard() {
  ...
  const { tickets, loading, createTicket, updateTicket, reassignTicket } = useTickets({
    assignedTo: isSuperAdmin ? undefined : employee?.id,
    isSuperAdmin,
  });
  ...
  const tabs = [
    ...
    ...(isSuperAdmin ? [{ id: 'reports' as Tab, label: 'Reports', icon: FileText }] : []),
  ];
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold text-primary">Tickets &amp; Reports</h2>
          <p className="text-sm text-secondary mt-0.5">
            {activeTab === 'tickets'
              ? isSuperAdmin ? `${tickets.length} total ticket${tickets.length !== 1 ? 's' : ''}` : 'Your assigned tickets'
      ...
      {activeTab === 'tickets' && (
        <>
          ...
          <div className="flex gap-4 flex-1 min-h-0">
            {/* Left: employee panel (superadmin only) */}
            {isSuperAdmin && (
              <div
                className="w-56 shrink-0 rounded-xl flex flex-col"
              ...
              >
                <div className="flex items-center gap-2 px-3 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <span className="text-xs font-medium text-secondary uppercase tracking-wider">Employees</span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto py-1">
                  <button
                    ...
                    style={{
                      background: selectedEmployeeId === 'all' ? 'rgba(0,212,255,0.1)' : 'transparent',
                      color: selectedEmployeeId === 'all' ? '#00d4ff' : 'rgba(255,255,255,0.7)',
                    }}
                  >
                  ...
                  {employees.map((emp) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {count > 0 && (
                          <span
                            className="text-xs px-1.5 py-0.5 rounded-full shrink-0 ml-1"
                            style={{
                              background: active ? 'rgba(0,212,255,0.2)' : 'rgba(255,255,255,0.1)',
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
              {loading ? (
                <div className="flex items-center justify-center h-48 text-secondary text-sm">Loading tickets…</div>
              ) : filtered.length === 0 ? (
